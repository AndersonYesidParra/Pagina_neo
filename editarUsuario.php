<?php
    ...
    if($_GET["user"]){
        ...
        $sql = "SELECT usuario.nombreUsuario, usuario.contrasenaUsuario, usuario.correoUsuario, rol.IdRol FROM usuario
        INNER JOIN rol ON rol.idRol = usuario.idRol WHERE usuario.idUsuario = {$user}";
        ...
        $sql = "UPDATE usuario SET nombreUsuario = '{$nombre}', contrasenaUsuario = '{$pass}', correoUsuario = '{$correo}', idRol = {$rol} WHERE idUsuario = {$user}";
...
<body>
    ...
    <script>
        function agregarDatos(nombre, correo, rol, password){
    ...
    <?php         
        echo "<script>";
        echo "agregarDatos('{$usuario[0]}', '{$usuario[2]}', {$usuario[3]} , '{$usuario[1]}');";
        echo "</script>";
    ?>
